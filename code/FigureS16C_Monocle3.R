...
data = readRDS('Batch1_Injury_control_FP200000239BL_E3.rds')
Idents(data) = 'inj_uninj'
data = subset(data,idents= 'inj')
Idents(data) = 'Annotation_0306'
s15_c = c('RIBEGC','SFRPEGC','WNTEGC','TLNBL','NPTXEX')
...
obj <- RunUMAP(obj, dims = 1:5, verbose = FALSE,min.dist=0.5)

obj <- FindNeighbors(obj, dims = 1:8, verbose = FALSE)
obj <- FindClusters(obj, verbose = FALSE)
saveRDS(obj,paste0('Batch1_Injury_control_FP200000239BL_E3.rds'))
...
p = DimPlot(obj, label = TRUE,group.by='Annotation_0306',cols=cor)
ggsave(plot=p,file=paste0('Annotation_0306.png'))

p = DimPlot(obj, label = FALSE,group.by='Annotation_0306',cols=cor)
ggsave(plot=p,file=paste0('Annotation_0306.pdf'))
...
get_earliest_principal_node <- function(cds, time_bin='RIBEGC'){
...
saveRDS(cds,paste0('Batch1_Injury_control_FP200000239BL_E3_monocle.rds'))
...
write.csv(meta,paste0('Batch1_Injury_control_FP200000239BL_E3_monocle.anno.csv'),quote=F)
