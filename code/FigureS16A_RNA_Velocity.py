...
involocito = 'Stage44_telencephalon_rep2_FP200000239BL_E4_WMPZY.loom'
inh5ad = 'Batch1_Stage44_telencephalon_rep2_FP200000239BL_E4_Annotation_0305.h5ad'
outpath = 'your path'
idname = 'Stage44_telencephalon_rep2_FP200000239BL_E4'
...
expressed_genes = (adata_spl_sel3.layers['spliced'].A + adata_spl_sel3.layers['unspliced'].A).sum(0) > 5 #(adata.layers['spliced'].A + adata.layers['unspliced'].A).sum(0) > 10000
...
dyn.tl.gene_wise_confidence(adata_spl_sel3, group='Annotation_0305', lineage_dict={'DEGC': ['Immature NPTXEX']})
...
dyn.tl.confident_cell_velocities(adata_spl_sel3, group='Annotation_0305', lineage_dict={'DEGC': ['Immature NPTXEX']},only_transition_genes=True,basis='spatial')

dyn.pl.cell_wise_vectors(adata_spl_sel3, color=['Annotation_0305'], basis='spatial', show_legend='on data', quiver_length=5, quiver_size=5, pointsize=0.1, show_arrowed_spines=False,color_key=co,figsize=(4, 4))
...
dyn.pl.streamline_plot(adata_spl_sel3, color=['Annotation_0305'], basis='spatial', show_legend='on data',color_key=co,
                       quiver_length=5, quiver_size=5,  show_arrowed_spines=True,background='black',pointsize=0.5,
...
plt.savefig('injury_D.kinetic_heatmap.spatial.leng.png')

heatmap = adata_spl_sel3.uns['kinetics_heatmap']
heatmap.to_csv('kinetics_heatmap.umap.csv')

dyn.pl.streamline_plot(adata_spl_sel3, color=['Annotation_0305','spatial_ddhodge_potential'], basis='spatial', show_legend='best',color_key=co,figsize=(5, 4),pointsize=0.6,background='black',cmap ='viridis',show_arrowed_spines=True)
