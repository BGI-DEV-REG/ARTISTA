...
library(tibble)
library(ggradar)
setwd('../data/Figure3_data/Figure3C')
...
color_cell <- read.table("../data/Figure3_data/Figure3C/Inj_24_Adult_Develop_color_0307.txt",comment.char = "*",header = T,sep = "\t")
...
gd1_long1<-melt(data_final,
                ...
                variable.name='time',
                value.name='ratio')
gd1_long1$time2 <- as.numeric(gd1_long1$time)
gd1_long1$time1 <- as.integer(gd1_long1$time)
p <- ggplot(gd1_long1,aes(x = time1 , y = ratio, colour = group,shape = group)) + 
  geom_line(size=0.4)+  
  geom_point(size=2.2)+ 
  scale_shape_manual(values = c(1,14,5,10,3,6,7,8,9,4,11,12,13,2)) + 
  theme_light()+
  scale_x_continuous(breaks=gd1_long1$time1, labels = gd1_long1$time) +
