...
setwd('../data/FigureS14D_Monocle3')
data = readRDS('Batch1_Injury_15DPI_rep3_FP200000266TR_E3.rds')
root = 'NPTXEX'
...
data = subset(data,idents= c('REAEGC','RIPC1','IMN'))

counts = data@assays$Spatial@counts
meta = data@meta.data
obj = CreateSeuratObject(counts= counts,meta.data =meta)
obj <- SCTransform(obj, verbose = FALSE)
obj <- RunPCA(obj, verbose = FALSE,npcs=7)
obj <- RunUMAP(obj, dims = 1:7, verbose = FALSE)

obj <- FindNeighbors(obj, dims = 1:7, verbose = FALSE)
obj <- FindClusters(obj, verbose = FALSE)
saveRDS(obj,paste0(root,'_Batch1_Injury_15DPI_rep3_FP200000266TR_E3.rds'))
...
saveRDS(cds,paste0(root,'_Batch1_Injury_15DPI_rep3_FP200000266TR_E3_monocle.rds'))
...
write.csv(meta,paste0(root,'_Batch1_Injury_15DPI_rep3_FP200000266TR_E3_monocle.anno.csv'),quote=F)
